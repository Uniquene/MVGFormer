...
class load_MVGsDataSetDGL(torch.utils.data.Dataset):
...
class MVGsDatasetDGL(torch.utils.data.Dataset):
    ...
    def __init__(self, name):
        ...
        self.name = name
        data_dir = 'data/MVGs'
        self.train = load_MVGsDataSetDGL(data_dir, name, split='train')
        self.test = load_MVGsDataSetDGL(data_dir, name, split='test')
        self.val = load_MVGsDataSetDGL(data_dir, name, split='val')
...
def laplacian_positional_encoding(g, pos_enc_dim):
    ...
    float_array = EigVec[:, 1:pos_enc_dim + 1].astype(np.float64)
    g.ndata['lap_pos_enc'] = torch.from_numpy(float_array).float()
    # 用degree作为节点特征
    # g.ndata['feat'] = torch.LongTensor(g.in_degrees().tolist())
...
def wl_positional_encoding(g):
    ...
    g.ndata['feat'] = g.in_degrees()
    return g



class MVGsDataset(torch.utils.data.Dataset):
    ...
    def collate(self, samples):
        ...
        batched_graph = dgl.batch(graphs)  #  batch into one graph
    ...
    def _add_self_loops(self):
        ...
        for i, item in enumerate(self.data):
            self.data[i][0] = self_loop(item[0])

        # self.data[0] = [self_loop(item[0]) for item in self.data]
        # self.val.graph_lists = [self_loop(g) for g in self.val.graph_lists]
        # self.test[0] = [self_loop(item[0]) for item in self.test]
    ...
    def _make_full_graph(self):
        ...
        # self.data[0] = [make_full_graph(item[0]) for item in self.data]
        # self.val.graph_lists = [make_full_graph(g) for g in self.val.graph_lists]
        # self.test[0] = [make_full_graph(item[0]) for item in self.test]

    def _add_laplacian_positional_encodings(self, pos_enc_dim):
        # Graph positional encoding v/ Laplacian eigenvectors
        for i, item in enumerate(self.data):
            self.data[i][0] = laplacian_positional_encoding(item[0], pos_enc_dim)

        # self.data[0] = [laplacian_positional_encoding(item[0], pos_enc_dim) for item in self.data]
        # self.val.graph_lists = [laplacian_positional_encoding(g, pos_enc_dim) for g in self.val.graph_lists]
        # self.test[0] = [laplacian_positional_encoding(item[0], pos_enc_dim) for item in self.test]

    def _add_wl_positional_encodings(self):
        # WL positional encoding from Graph-Bert, Zhang et al 2020.
        for i, item in enumerate(self.data):
            self.data[i][0] = wl_positional_encoding(item[0])

        # self.data[0] = [wl_positional_encoding(item[0]) for item in self.data]
        # self.val.graph_lists = [wl_positional_encoding(g) for g in self.val.graph_lists]
        # self.test[0] = [wl_positional_encoding(item[0]) for item in self.test]
